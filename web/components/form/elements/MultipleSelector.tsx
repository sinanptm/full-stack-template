...
const CommandEmpty = React.memo(({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.Empty>) => {
...
const MultipleSelector = React.forwardRef<HTMLInputElement, MultipleSelectorProps>(
    (
        ...
        const handleSelect = useCallback((option: Option) => {
            ...
            // Check if we've reached max after adding
            if (newOptions.length === maxSelected) {
                onMaxSelected?.(newOptions.length);
                // Don't close dropdown or blur input - allow continued interaction
        ...
        const CreatableItem = useMemo(() => {
            ...
            return null;
        }, [creatable, inputValue, options, selected, onSearch, debouncedSearchTerm, isLoading, maxSelected, onMaxSelected, handleOptionsChange]);
        ...
        return (
            <div className="space-y-2  [--ring:var(--color-indigo-300)] *:not-first:mt-2 in-[.dark]:[--ring:var(--color-indigo-900)]">
                ...
                <Command
                    ...
                    shouldFilter={commandProps?.shouldFilter !== undefined ? commandProps.shouldFilter : searchable && !onSearch}
                    ...
                    >
                        <div className="flex flex-wrap gap-1">
                            ...
                            <CommandPrimitive.Input
                                ...
                                aria-describedby={cn(error && `${id}-error`, hint && showHint && !error && `${id}-hint`)}
                                    ...
                                    {isLoading ? (
                                        ...
                                        <>
                                            ...
                                            {Object.entries(selectables).map(([key, dropdowns]) => (
                                                <CommandGroup key={key} heading={key} className="h-full overflow-auto cursor-pointer">
                                                    {dropdowns.map((option) => {
                                                        const canSelect = !isMaxSelected || selected.find(s => s.value === option.value);
                                                        return (
                                                            <CommandItem
                                                                key={option.value}
                                                                value={option.value}
