...
describe("OAuthUseCase", () => {
    ...
    describe("exec", () => {
        it("should authenticate new user successfully and return tokens", async () => {
            ...
            const user = { _id: "user123", email: validPayload.email, name: validPayload.name, profile: validPayload.profile, isOAuthUser: true, isBlocked: false };
        ...
        it("should authenticate existing user successfully and return tokens", async () => {
            const oauthUser = { uid: "oAuthUser123", email: validPayload.email, name: validPayload.name, profile: validPayload.profile };
            const user = { _id: "user123", email: validPayload.email, name: validPayload.name, profile: validPayload.profile, isOAuthUser: true, isBlocked: false };
            const accessToken = "jwtAccessToken";
            const refreshToken = "refreshToken";
        ...
        it("should throw UnauthorizedError for email mismatch", async () => {
            const oauthUser = { uid: "oAuthUser123", email: "different@example.com", name: validPayload.name, profile: validPayload.profile };
            mockOAuthService.verifyAccessToken.mockResolvedValue(oauthUser);
